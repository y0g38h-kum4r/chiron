...
impl Snapshot {
    ...
    pub fn save(
        ...
        write_u64(&mut buf, ring_buffer.capacity() as u64);
        write_u64(&mut buf, ring_buffer.next_offset()); // global_offset
        write_u64(&mut buf, ring_buffer.len() as u64);

        // Ring buffer entries (oldest to newest)
        ...
        // Kafka offsets
        write_u32(&mut buf, kafka_offsets.offsets.len() as u32);
        for (topic, partitions) in &kafka_offsets.offsets {
            write_string(&mut buf, topic);
            write_u32(&mut buf, partitions.len() as u32);
            for (&partition, &offset) in partitions {
                write_u32(&mut buf, partition);
    ...
    pub fn load(path: &Path) -> io::Result<(RingBuffer, KafkaOffsets)> {
        ...
        let global_offset = read_u64(&mut cursor)?;
        ...
        // Reconstruct ring buffer.
        let rb = RingBuffer::restore(capacity, global_offset, len, entries);

        // Kafka offsets
        let topic_count = read_u32(&mut cursor)? as usize;
        let mut kafka_offsets = KafkaOffsets::new();
        for _ in 0..topic_count {
            let topic = read_string(&mut cursor)?;
            let partition_count = read_u32(&mut cursor)? as usize;
            for _ in 0..partition_count {
                let partition = read_u32(&mut cursor)?;
