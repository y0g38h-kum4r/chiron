...
impl PartitionShard {
    ...
    fn snapshot_shard(&self) -> SnapshotShard {
        let inner = self.inner.read().unwrap();
        SnapshotShard {
            shard_id: self.shard_id,
            capacity: self.capacity,
            next_offset: inner.buffer.next_offset(),
            entries: inner.buffer.entries_owned(),
        }
    }
}
...
mod tests {
    ...
    fn snapshot_and_restore() {
        let dir = unique_test_dir("chiron_test_store_snapshot");
